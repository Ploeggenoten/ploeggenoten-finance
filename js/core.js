...
const D = {            // alle data, geladen in loadAll()
  ...
  dismissed: [], candidates: [], clients: [], flex: [], targets: [], tarieven: [], yukiOpen: [], flexPl: [], flexAfspr: [], flexRegels: [],
};
...
async function loadAll() {
  ...
  const [pl, inst, bud, act, sal, tx, ln, lp, st, dis, fx, trf, yo, fpl, faf, freg, cand, cli, tgt] = await Promise.all([
    ...
    /* Regels van de Pronkert-margefacturen: per flexkracht per dag wat we
       verdienen, met uurloon, inkoopfactor en klanttarief. Tolerant geladen —
       is de migratie niet gedraaid, dan werkt de rest gewoon door. */
    q('fin_flex_regels', { col: 'week' }),
    q('candidates'),
    q('clients'),
    q('targets'),
  ]);
  ...
  D.flexRegels = freg.error ? [] : freg.data;
